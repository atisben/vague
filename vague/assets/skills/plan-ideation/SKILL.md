...
**If engineering or CEO plans already exist:** Warn the user: "There are existing plans for this project: [filenames]. Starting a new plan-ideation session may produce a design doc that conflicts with them. Should I read them first for context, or are you exploring a fresh direction?"
...
Generated by /plan-ideation on {date}  |  Branch: {branch}  |  Mode: Startup  |  Status: DRAFT
...
Generated by /plan-ideation on {date}  |  Branch: {branch}  |  Mode: Builder  |  Status: DRAFT
...
```bash
vague learnings-log '{"skill":"plan-ideation","type":"pattern","key":"SHORT_KEY","insight":"INSIGHT","confidence":7,"source":"inferred"}'
```
...
> Run `/plan-ceo` to pressure-test scope and find the 10-star version. Then `/plan-eng` before writing a line of code."
...
> When you've validated that: `/plan-ceo` to lock scope, then `/plan-eng`."
...
> When you're ready: `/plan-eng` to lock in the architecture, then `/dev-ship`. Don't over-plan it — the best builder projects are started, not planned."
...
```bash
vague observations-log '{"skill":"plan-ideation","type":"improvement","issue":"WHAT_HAPPENED","suggestion":"CONCRETE_CHANGE","principle":"GENERALISABLE_TAKEAWAY","source_skill":"plan-ideation"}'
```
...
```bash
vague observations-log '{"skill":"plan-ideation","type":"correction","issue":"User had to manually ask: WHAT_THEY_SAID","suggestion":"Add this as an explicit step in the skill","principle":"GENERALISABLE_TAKEAWAY","source_skill":"plan-ideation"}'
```
