...
```bash
~/.bastack/bin/bs-timeline-log "{\"skill\":\"review\",\"event\":\"completed\",\"branch\":\"$(git branch --show-current)\",\"outcome\":\"success\",\"session\":\"$SESSION_ID\"}"
```

Log any non-obvious pattern discovered:
```bash
~/.bastack/bin/bs-learnings-log '{"skill":"review","type":"pitfall","key":"SHORT_KEY","insight":"INSIGHT","confidence":8,"source":"observed","files":["path/to/file"]}'
```
