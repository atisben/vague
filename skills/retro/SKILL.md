...
ls -t "$BASTACK_HOME/projects/$SLUG/retros/"*.md 2>/dev/null | head -1 || echo "NO_PRIOR_RETRO"
```
...
```bash
~/.bastack/bin/bs-learnings-search --limit 5 2>/dev/null || true
```
...
eval "$(~/.bastack/bin/bs-slug 2>/dev/null)"
BASTACK_HOME="${BASTACK_HOME:-$HOME/.bastack}"
mkdir -p "$BASTACK_HOME/projects/$SLUG/retros"
RETRO_FILE="$BASTACK_HOME/projects/$SLUG/retros/retro-$(date +%Y-%m-%d).md"
...
Log telemetry:
```bash
~/.bastack/bin/bs-timeline-log "{\"skill\":\"retro\",\"event\":\"completed\",\"branch\":\"$(git branch --show-current 2>/dev/null)\",\"session\":\"$SESSION_ID\"}"
```
