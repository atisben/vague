...
```bash
~/.bastack/bin/bs-timeline-log "{\"skill\":\"ship\",\"event\":\"completed\",\"branch\":\"$BRANCH\",\"outcome\":\"success\",\"session\":\"$SESSION_ID\"}"
```

Log any learnings discovered during implementation:
```bash
# If a non-obvious pattern or pitfall was encountered:
~/.bastack/bin/bs-learnings-log '{"skill":"ship","type":"pitfall","key":"SHORT_KEY","insight":"INSIGHT","confidence":8,"source":"observed"}'
```
